//READ 1 DATA WITH DUMMY 8 CYCLES
`timescale 1ns/1ps
...
module tb_spi;
  ...
  always@(posedge HCLK)begin
    $display("CMD = %h ADR = %h CMD_LEN = %h ADR_LEN = %h DATA_LEN = %h WR = %b RD = %b STAT = %h",
      dut.spi_cmd, dut.spi_addr, dut.spi_cmd_len, dut.spi_addr_len, dut.spi_data_len, dut.spi_wr, dut.spi_rd, dut.spi_ctrl_status);
  end
    ...
    initial begin
        ...
        apb_write(REG_SPICMD, 32'hA5000000); // Command = 0xA5 (MSB Aligned)
        apb_write(REG_SPIADR, 32'h5A000000); // Address = 0x5A (MSB Aligned)
        
        $display("[TB] Setting Transfer Lengths (CMD=8b, ADDR=8b, RX_DATA=32b)...");
        apb_write(REG_SPILEN, 32'h00200808); 
      	apb_write(REG_SPIDUM, 32'h0000_0008); //DUMMY 8 CYCLES
      
        $display("[TB] Triggering Standard SPI Read Transaction...");
        // PWDATA[0] = 1 (spi_rd), PWDATA[11:8] = 4'b0001 (CS0)
      	apb_write(REG_STATUS, 32'h00000201);

        // Step 4: Safely await the core event return signal from the DUT
        $display("Awaiting transaction completion...");
        @(posedge HCLK); 
        wait(events_o[1] == 1'b1); // Wait for EOT (End of Transaction)
        $display("SPI transaction completed at time %0t", $time);
        
        // Step 5: Read the captured data out of the RX FIFO via APB
