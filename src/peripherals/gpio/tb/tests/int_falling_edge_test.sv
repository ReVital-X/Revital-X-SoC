class int_falling_edge_test extends gpio_base_test;

    logic [31:0] rdata;

    function new(apb_driver master);
        super.new(master);
        test_name = "INT_FALLING_EDGE_TEST";
    endfunction

    task run();

      $display("\n========== INT FALLING EDGE TEST ==========");
          ...
          master.write(REG_INTTYPE0, 32'h1 << bt);
